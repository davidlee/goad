...
|----------|--------|--------|------|
| `docs/adr/002-single-crate-until-triggered.md` → **new** `docs/adr/003-*.md` (CD-1) | supersede: record the decision as taken (four crates along the strata, plus `goad-boundary`), the four things ADR-002 left open now decided, what the split bought and exactly where that claim stops, what it cost (measured: 113 renames, 92 byte-identical, 24 additions, 3 deletions, 4 modifications), and where ADR-001's discipline actually slipped. **Not yet drafted as an ADR file** — CD-1's content lives in `canon-delta.md` only; `docs/templates/adr.md` is the template to draft it from, and drafting requires explicit user endorsement of the wording (the *decision* is already endorsed, `design-log.md` 2026-09-05; the wording is not) | ADR-002's own Verification section requires supersession, not amendment, the day the split lands; `docs/adr/002-…md:3` still reads `accepted` against a workspace that exists | [ ] |
| same new ADR (CD-2) | fold in, as the trigger record: ADR-002's stated reason for T1 ("a build-dependency with a conditional `build.rs` cannot be gated as cleanly") is false, measurably so — an optional `[build-dependencies]` entry plus `#[cfg(feature = "ui")]` resolves to one node. State the two real reasons T1 fired instead (the dev-dependency; the lint collision) | a superseded ADR that hides its own false premise invites a future slice to reason from it | [ ] |
| `docs/specs/001-host-backend-protocol.md` §2 and a new renderer-facing requirement (CD-3) | close the gap: a renderer MUST NOT refuse a view because it cannot draw part of it; it draws what it can and reports what it did not. Adjust §2's out-of-scope wording: drawing stays out of scope, what a renderer owes the protocol does not | R-20's guarantee stops at normalization; §2 leaves nothing binding the renderer, and this slice is the first renderer — CLAUDE.md invariant 3's failure mode, closed before it could occur in code | [ ] |
| `docs/specs/001-host-backend-protocol.md` §7 (CD-4) | restate the fixture path as `tests/fixtures/`, not `tests/protocol/fixtures/` | the split moved the directory so both crates can reach it; a normative path went stale by relocation | [ ] |
| `CLAUDE.md` Verifying section (CD-5) | repoint "the command block in `docs/slices/001/design.md` §9 is canonical" at the promoted phase-gate policy; leave `docs/slices/001/design.md` §9 untouched as slice 001's record | a closed slice's design cannot be the gate's live definition without being retro-fitted, which `docs/AGENTS.md` forbids; the split changed the gate's scope, forcing the choice | [ ] |
| `CLAUDE.md` invariant 1 (CD-6) | **no wording change**, contingent on D13/D17 as implemented: the boundary scan is confirmed to read `workspace.members` and to be string-literal-aware across `.rs` and `.slint` (`vocabulary.rs`, checked live above). If this were ever dropped, this becomes a `CLAUDE.md` amendment instead | `CLAUDE.md`'s claim that "a boundary test greps for it" would be false the day markup existed and the test still scanned `.rs` only; it does not, so the claim stands as written | [ ] |
| `CLAUDE.md` Verifying section (CD-7) | replace "clippy in **both** feature columns" and "a matrix checked in one column is unchecked" with a pointer to the phase-gate policy and the three-part count (four ADR-001 instruments, the vocabulary scan, one unenforced residue) | confirmed false today: `justfile:50` carries one clippy line, no feature gates `tokio`/`toml` out of stratum 1 | [ ] |
| `draft-policy.md` → `docs/policy/NNN-the-phase-gate.md` | promote | drafted this slice as the phase gate's new canon (`docs/templates/policy.md`); CD-5 and this promotion land together or not at all — applying CD-5 alone leaves `CLAUDE.md` pointing at nothing, promoting the policy alone leaves two claimants to the gate | [ ] |
