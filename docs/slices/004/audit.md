...
Every row needed explicit user endorsement except the last two, which amend the
slice's own documents rather than canon. **The user endorsed all five canon
rows on 2026-09-10, and all seven are applied.**

| document | change | reason | done |
|----------|--------|--------|------|
| `docs/specs/002-host-scheduling-behaviour.md` | apply `canon-delta.md` **CD-1** — add **SPEC-002/R-12**, the event bound and the principle it instances; amend §2's count of what the spec abuts and §6's sentence naming what the host owns | R-5 required a host adding a stimulus other than a due check to decide separately how it is bounded. This slice decided it; the spec must now say so | [x] — §2 Boundaries restated as *two points with SPEC-001, one with SPEC-003*; §3 gains **P-E**, the general principle R-4 and R-12 are the two instances of; §4 gains **R-12**; §5 gains the paragraph on what the ingested bound looks like from outside and the OQ-4 rate sentence; §6 names the two anchors and *one constant per bounded stimulus class*; §7 gains R-12's row, in two parts — the bound, and the anchors' independence in all three directions; §9 gains SPEC-003, ADR-005 and slice 004's design |
| `docs/specs/001-host-backend-protocol.md` §R-56 | apply `canon-delta.md` **CD-2** — narrow the first clause to every `evaluate` the host originates *on its own account*, and add the clause reserving `"host"` as a source | without the narrowing a conforming host breaches R-56 the moment it forwards an ingested event. The matching *refusal* is SPEC-003's, not this spec's | [x] — R-56's first clause now reads *"originates **on its own account**"*, with a sentence saying why the qualifier is load-bearing; the reserved-source clause is added, granting a backend the reading and pointing at SPEC-003/R-13 for what the grant rests on; R-56's §7 row gains the acceptance-side verification and declares the emission half *review, not a test*; §9 gains SPEC-003 |
| `docs/adr/004-scheduled-firings-are-spaced-from-the-previous-scheduled-firing.md` §Verification | apply `canon-delta.md` **CD-3** — name the three AC-6 tests by file and function, saying which discharges the debt; add `design.md` §5.3 and §9 to References. The decision itself is untouched | the ADR says the case that separates the anchor from the boolean is the one slice 004 will introduce. `renderer/ingress.rs::an_ingested_firing_does_not_advance_the_scheduled_floor` is that case, and it passes and has been shown to fail when the anchor is broken | [x] — §Verification's second paragraph now states the anchor **is** verified, quotes the discharging case by file and function with the clause that makes it the right case, and names the two siblings as what they are rather than letting them look like the discharge. What stays with review is narrowed to the choice of anchor for stimulus classes that do not yet exist, pointing at SPEC-002 §3 P-E. The decision is untouched |
| `docs/slices/004/draft-spec.md` → `docs/specs/003-host-event-ingress.md` | promote as **SPEC-003**, taking its number at promotion | drafted during this slice as its working authority; §7's sixteen rows each name a real test function and file, checked here, and R-5 correctly reads *review, not a test*. `docs/AGENTS.md`: a slice does not close holding an unpromoted draft. **Promote §6.3's closing sentence as a criterion, not as a row note** (`review-code.md` F-15): *"A clause of this paragraph that says a refusal always reaches a person is making a claim about who decides it… One that cannot say which side decides is a clause that has not been checked."* It is what makes the next instance of that class findable by reading, and F-15 found two instances of it in one sweep | [x] — moved to `docs/specs/003-host-event-ingress.md`, **SPEC-003**, status active. §6.3's closing sentence is promoted into **§3 P-D** as a stated criterion binding the whole document and its future amendments, not a note on one paragraph; §6.3 keeps the specific application. Two Verification rows were rewritten on promotion to strip the document's own editing history — canon *"states what is true now"*, and R-1's and R-5's rows had accumulated accounts of how they had previously been wrong. §7's preamble drops its slice framing. Code and `examples/demo.toml` citations repointed from `draft-spec.md` to `SPEC-003` |
| `docs/adr/005-…md` (**new**) | write: *the event envelope normalizes in stratum 2.* | ADR-001 §Decision names wire-to-canonical normalization in stratum 1 and event ingress in stratum 2, and the envelope is both. `design.md` D-3 records the decision and §10 says it is owed an ADR, because **no ADR-001 instrument sees this choice** — it could be reversed by accident | [x] — `docs/adr/005-the-event-envelope-normalizes-in-stratum-2.md`, accepted 2026-09-10. Records what decides the placement (*which contract a normalization holds, not which shape it has*), why the "only door" invariant does not forbid it, three rejected alternatives, and — under Consequences, Neutral — that **no ADR-001 instrument sees this choice**, so the gate's silence here is not approval |
