...
| 10 | **`today_local`'s system zone is held by review alone.** `instant.rs` reads the system zone to seed an unpicked `datetime` picker, and that the zone is the **system's** is asserted by nothing — the class is covered (`one_instant_is_two_different_local_dates_in_two_different_zones`, injections I2/J/K), the token is not | Same class as #3 and #4 — real, and held by nothing — and it is here for the same reason they are: a unit cannot change the zone the machine is in, so holding it needs a harness that can, not a case. `instant.rs`'s own doc states the gap at `today_local_answers_midnight_on_the_date_the_system_zone_is_having`. Found at close by the `notes.md` §Open sweep |
