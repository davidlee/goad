...
| AC-8, **as measured** | unchanged | **the click on the `ListItem` was not available and the row did not have to move** (PHASE-09/VA-1; §8 R9 as measured). A popup's items are addressed in the popup's own coordinate space and `mock_single_click` dispatches in the window's, which no tier changes. The driver is a `mock_single_click` on the **`ComboBox` itself** plus key events — so the two fallbacks the AC-2 row inherits were never reachable either, and neither row moved | unchanged |
