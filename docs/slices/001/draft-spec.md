...
| R-46 | clippy denying `unwrap_used`, `expect_used` and `indexing_slicing` crate-wide in `[lints.clippy]`, and `arithmetic_side_effects` per-module on the modules that handle backend-derived data. The blanket form is what F-35 caught the design violating on a post-spawn stdio handle — a value the host itself created — and `allow_attributes_without_reason = "deny"` is what makes that survivable: the exception is a written reason at the site rather than a silent `#[allow]`. `arithmetic_side_effects` stays scoped because crate-wide it fires on every loop counter. **Crate-wide stops at the test targets**: `clippy.toml` sets `allow-unwrap-in-tests`, `allow-expect-in-tests`, `allow-panic-in-tests` and `allow-indexing-slicing-in-tests`, because otherwise ordinary test code fails the gate — measured. `unwrap_in_result` is not carved out. R-46 constrains host paths, and a test that unwraps is asserting (D53 as amended 2026-08-27; F-14) |
