{
  ...
  outputs = inputs @ {
    ...
    # SPIKE: one toolchain binding, shared by the devshell and crane —
    # doctrine's flake records that crane's default (nixpkgs-stable rustc)
    # flips lint verdicts against a beta gate.
    rust = pkgs.rust-bin.beta.latest.default;

    craneLib = (crane.mkLib pkgs).overrideToolchain rust;

    # SPIKE: cleanCargoSource keeps only .rs/.toml/.lock; build.rs compiles
    # ui/app.slint, which imports two .ttf faces. Widen rather than graft.
    ...
    # SPIKE FINDING: `builtins.fromTOML` (Lix 2.95.2) refuses this workspace
    # manifest — `[workspace.dependencies]` writes `tokio = { version = "1",`
    # across two lines, and a newline inside an inline table is TOML 1.1, which
    # the parser does not accept. doctrine's `fromTOML`-reads-the-version
    # pattern therefore does not transfer as written. Literal for the spike.
    ...
    packages.${system} =
      ...
      // {
        goad = craneLib.buildPackage {
          ...
          cargoExtraArgs = "-p goad --bin goad";
          nativeBuildInputs = [pkgs.pkg-config pkgs.makeWrapper];
          buildInputs = guiLibs;
          doCheck = false; # SPIKE arm A
          postInstall = ''
        ...
        # SPIKE arm B1: doCheck = true, no fonts configuration in the check
        # phase. `cargoExtraArgs` is inherited by crane's checkPhase, so it
        # must NOT carry `--bin goad` — that would test the binary target
        # alone and report green having run nothing.
        goad-checked-nofonts = craneLib.buildPackage {
          pname = "goad-checked-nofonts";
          version = workspaceVersion;
          inherit src cargoArtifacts;
          cargoExtraArgs = "--locked -p goad";
          nativeBuildInputs = [pkgs.pkg-config pkgs.makeWrapper];
          buildInputs = guiLibs;
          doCheck = true;
          meta.mainProgram = "goad";
        ...
        # SPIKE arm B2: the same, with the fonts configuration the devshell and
        # the jail both set.
        goad-checked-fonts = craneLib.buildPackage {
          pname = "goad-checked-fonts";
          version = workspaceVersion;
          inherit src cargoArtifacts;
          cargoExtraArgs = "--locked -p goad";
          nativeBuildInputs = [pkgs.pkg-config pkgs.makeWrapper];
          buildInputs = guiLibs;
          doCheck = true;
          # SPIKE: the sandbox has neither. Without TZDIR six `instant.rs`
          # cases fail — jiff's `tzdb-zoneinfo` finds no database; without
          # FONTCONFIG_FILE the renderer tier cannot construct a component.
          FONTCONFIG_FILE = fontsConf;
          TZDIR = "${pkgs.tzdata}/share/zoneinfo";
          meta.mainProgram = "goad";
        };

        # SPIKE negative control: crane's stock source filter, unwidened.
        # If this builds, widening the filter is unnecessary.
        goad-narrow = craneLib.buildPackage {
          pname = "goad-narrow";
          version = workspaceVersion;
        ...
        # SPIKE: .slint admitted, .ttf still stripped — isolates which half of
        # the widening the fonts actually need.
        goad-slint-only = craneLib.buildPackage {
          pname = "goad-slint-only";
          version = workspaceVersion;
          src = lib.cleanSourceWith {
            src = ./.;
            filter = path: type:
              (craneLib.filterCargoSources path type) || (lib.hasSuffix ".slint" path);
            name = "goad-source-slint-only";
          };
          inherit cargoArtifacts;
          cargoExtraArgs = "--locked -p goad --bin goad";
          nativeBuildInputs = [pkgs.pkg-config pkgs.makeWrapper];
        ...
        # SPIKE: identical to `goad` but a distinct derivation, to time the
        # final layer alone against warm `cargoArtifacts`.
        goad-remeasure = craneLib.buildPackage {
          pname = "goad-remeasure";
          version = workspaceVersion;
          inherit src cargoArtifacts;
          cargoExtraArgs = "--locked -p goad --bin goad";
          nativeBuildInputs = [pkgs.pkg-config pkgs.makeWrapper];
        ...
        goad-emit = craneLib.buildPackage {
          ...
          version = workspaceVersion;
          inherit src cargoArtifacts;
          cargoExtraArgs = "-p goad-emit --bin goad-emit";
          nativeBuildInputs = [pkgs.pkg-config];
          buildInputs = guiLibs;
          doCheck = false;
