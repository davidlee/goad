...
|---|---|
| R-1 | integration: a running host with a backend that supplies no `next_check` performs a second `evaluate` about one default poll after the first, evidenced by the backend's own invocation record |
| R-2 | structural: a scan asserting the resolution function has no call site outside the one the host reports from |
| R-3 | unit, on the pure wait computation: an instant at `now`, before `now`, and at the edge of representable time, at both floors. Integration: a backend answering with a past instant every time is invoked a bounded number of times over an interval far shorter than the spacing |
| R-4 | integration: a backend answering with a past instant every time yields a bounded invocation count over a window far shorter than the spacing, and a person acting in the middle of a scheduled cadence does not raise that count |
| R-5 | integration: the first scheduled evaluation of the process honours a default poll shorter than the spacing; an evaluation a person asks for is dispatched without waiting for it |
| R-6 | integration: what the host reports as its next check after a past instruction is the instruction, unchanged — the SPEC-001/R-28 assertion, re-read after this document exists |
| R-7 | integration: an instruction earlier than the pending check shortens the wait, observed as a firing inside a bound; one later lengthens it, observed as no firing inside a window |
| R-8 | integration: a clock that fails produces the refusal and a bounded invocation count, and the retained check is unchanged |
| R-9 | structural and by construction: the wait is one arm of a loop that runs one exchange at a time |
| R-10 | integration: a stop request while a wait is pending ends the loop promptly |
| R-11 | **review, not a test** — nothing persists, so there is no record of a missed interval to catch up from. The requirement records the intent for the slice that adds persistence |
